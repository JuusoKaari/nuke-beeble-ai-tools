...
"""Pack nuke-beeble-ai-tools as an install tree, not a development checkout.

Run without a git tag:

    python .github/scripts/pack_release.py

Default output is dist/nuke-beeble-ai-tools-dev.zip. Tests import this module and
...
# User docs only: INSTALL. Maintainer-only docs stay out of the archive.
REQUIRED_FILES = (
    "init.py",
    "menu.py",
...
def default_archive_path(repo_root, version=DEFAULT_VERSION):
    name = "nuke-beeble-ai-tools-%s.zip" % version
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Pack the nuke-beeble-ai-tools install ZIP from an allowlist."
    )
    parser.add_argument(
        "--root",
    ...
    parser.add_argument(
        "--output",
        default="",
        help="Zip path. Default: <root>/dist/nuke-beeble-ai-tools-<version>.zip",
    )
